...
export const SessionsPage = () => {
  const [sessionsState, setSessionsState] = useState<{
    sessions: (GameSessionExpanded & { studentCount: number })[];
    loading: boolean;
  }>({
    sessions: [],
    loading: true,
  ...
  const { sessions, loading } = sessionsState;

  useEffect(() => {
    const fetchSessions = async () => {
      const data = await sessionService.getUserSessions();
      setSessionsState({ sessions: data, loading: false });
    };

    fetchSessions();

    const subscription = sessionService.subscribeToAllUserSessions(() => {
      fetchSessions();
    });
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-2xl border border-dashed border-gray-300 dark:border-gray-700">
          ...
          <p className="text-gray-500 max-w-sm mx-auto mb-6">
            Create a game session to share with your students or friends and
            track their scores.
          </p>
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div
                    className={`px-2.5 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
                      sessionService.isSessionActive(session)
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
                  ...
                  <span className="truncate">
                    {session.expand?.game?.title || "Unknown Game"}
                  </span>
                </div>
